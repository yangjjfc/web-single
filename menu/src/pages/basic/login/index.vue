...
export default {
    ...
    methods: {
        ...
        // 微信登录
        weixinLogin () {
            let redirectUrl = window.location.origin + '/loginByWx';
            let timestamp = new Date().getTime();
            sessionStorage.setItem('SCS_WX_STATE', timestamp + '');
            window.location = 'https://open.weixin.qq.com/connect/qrconnect?appid=wx590b2602f61063ca&redirect_uri=' + encodeURIComponent(redirectUrl) + '&response_type=code&scope=snsapi_login&state=' + timestamp + '#wechat_redirect';
        },
        // 登录
        login () {
            let xflag = false;
