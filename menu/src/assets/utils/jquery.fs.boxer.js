 ...
 * @version 3.0.0
 */
import load_error from "@/assets/images/load_error.png";
(function ($, window) {
    'use strict';
    ...
    function _build (e) {
        ...
        var $target = $(this),
            ...
            isPdf = (type === 'image') || (extension === 'pdf'),
            isVideo = (source.indexOf('youtube.com/embed') > -1 || source.indexOf('player.vimeo.com/video') > -1),
            isUrl = ((type === 'url') || (!isImage && !isPdf && !isVideo && source.substr(0, 4) === 'http')),
            isElement = ((type === 'element') || (!isImage && !isPdf && !isVideo && !isUrl && source.substr(0, 1) === '#')),
            isObject = ((typeof $object !== 'undefined'));

        // Check if boxer is already active, retain default click
        if ($('#boxer').length > 1 || !(isImage || isVideo || isUrl || isElement || isObject || isPdf)) {
            return;
        }
        ...
        if (isImage || isVideo || isPdf) {
            ...
            if (data.gallery.active) {
                ...
                html += '>';
                html += '<span class="current">' + (data.gallery.index + 1) + '</span>' + data.labels.count + '<span class="total">' + (data.gallery.total + 1) + '</span><span>张</span>';
                html += '</p>';
            ...
            // 自定义添加
            if (data.$target.data('print')) {
                html += '<p style="text-align: center">';
                html += '<button class="btn_print btn_boxer_quire" style="margin-right: 20%" >确认使用</button>';
                html += '<button class="btn_print btn_boxer_cancel">取消</button>';
                html +=	'</p>';
            }
        ...
        data.$boxer.stop().animate({ opacity: 1 }, data.duration, function () {
            if (isImage) {
                _loadImage(source, 'img');
            } else if (isPdf) {
                _loadImage(source, 'pdf');
            } else if (isVideo) {
                _loadVideo(source);
            } else if (isUrl) {
                _loadURL(source);
	 ...
	 */
    function _loadImage (source, type) {
        // Cache current image
        if (type == 'img') {
            data.$image = $('<img onerror="javascript:this.src=\'../images/load_error.png\';"/>');
        } else {
            data.$image = $('<iframe type="application/pdf" ><iframe>');
        }
        // data.$image.one("load.boxer", function() {
        //	var naturalSize = _naturalSize(data.$image);
        //	if(type=='img'){
        //		data.naturalHeight = naturalSize.naturalHeight||400;
        //		data.naturalWidth  = naturalSize.naturalWidth||400;
        //	}else{
        //		data.naturalHeight=800;
        //		data.naturalWidth=1500;
        //	}
        //	data.naturalHeight = naturalSize.naturalHeight||400;
        //	data.naturalWidth  = naturalSize.naturalWidth||400;
        //
			...
			if (data.$image[0].complete || data.$image[0].readyState === 4) {
                ...
                setTimeout(function () {
					if (data.$image[0].complete || data.$image[0].readyState === 4) {
                        ...
                        data.$image.attr('src', load_error);
                        load();
                    }
                }, 5000);
            }
    ...
    function _advanceGallery (e) {
        ...
        if (!$arrow.hasClass('disabled')) {
            ...
            data.$content.stop().animate({ opacity: 0 }, data.duration, function () {
                ...
                var str = '';
                // if(data.$target.data('print')){
                //	str +='<p style="text-align: center"><button class="btn_print">打印</button></p>'
                // }
                //
                data.$caption.html(data.formatter.apply(data.$body, [data.$target]) + str);
                data.$position.find('.current').html(data.gallery.index + 1);

                var source = data.$target.attr('href'),
                    isVideo = (source.indexOf('youtube.com/embed') > -1 || source.indexOf('player.vimeo.com/video') > -1),
                    isPdf = (source.indexOf('.pdf') > -1);
                if (isVideo) {
                    _loadVideo(source);
                } else if (isPdf) {
                    _loadImage(source, 'pdf');
    ...
    function _loadURL (source) {
        source += ((source.indexOf('?') > -1) ? '&' + options.requestKey + '=true' : '?' + options.requestKey + '=true');
